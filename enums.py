...
# write out ruby file
with open(root + '/ruby/lib/yoga_layout/bindings/enums.rb', 'w') as f:
    value_tranform = lambda x: to_java_upper(x).lower()
    f.write('# This file is auto-generated by //enums.py\n')
    f.write('# Do not edit by hand!\n')
    f.write('module YogaLayout\n')
    f.write('  module Bindings\n')
    for name, values in sorted(ENUMS.items()):
        f.write('\n    %s = enum(\n' % (name))
        for value in values:
            if isinstance(value, tuple):
                f.write('      :%s, %d,\n' % (value_tranform(value[0]), value[1]))
            else:
                f.write('      :%s,\n' % (value_tranform(value)))
        f.write('    )\n')
        f.write('    typedef %s, :YG%s\n' % (name, name))
    f.write('\n  end\n')
    f.write('end\n')
